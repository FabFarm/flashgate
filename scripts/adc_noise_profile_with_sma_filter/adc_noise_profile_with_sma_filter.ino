...
//  Serial.println(step_adc_last);

//  Serial.println( (String) noise_profile.mean + "," + 
//                  (String) line_stdev_plus + "," + 
//                  (String) line_stdev_min);
                  
  Serial.println( (String) noise_profile.mean + "," + 
                  (String) line_stdev_plus + "," + 
                  (String) line_stdev_min + "," +
                  (String) step_adc_filt + ",4095,0");
...
void apply_sma_filter(){
  ...
  for(int i = 0; i < NR_SAMPLES_SMA; i++){
    ...
    // If value pointer is negative, get values from other side of the ring buffer
    if ( (pointer_rb_adc - i) < 0 ){
      i_rb = NR_SAMPLES_RB_ADC + (pointer_rb_adc - i);
    }
    else{
      i_rb = pointer_rb_adc - i;
    }
